...
- `label` is optional: a human-readable name supplied by the URI creator; it is unauthenticated. It is intended for display in the payer's wallet and is not sent to the recipient. The value is percent-encoded UTF-8 and MUST be at most 255 bytes after percent-decoding (a longer value makes the entire URI invalid); an empty value is treated as absent.
- `message` is optional: a payment description supplied by the URI creator; it is unauthenticated. It is intended for display to the payer and is not sent to the recipient. The value is percent-encoded UTF-8 and MUST be at most 255 bytes after percent-decoding (a longer value makes the entire URI invalid); an empty value is treated as absent.
